...
##sp500 = ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "XEL","WELL"] ##Sample data
...
def analyze_stock(ticker):
    try:
        ...
        price_above_50_ma = latest['Close'][ticker] > stock_data.iloc[-1]['50_MA'].iloc[0]
        
        # Check if the stock's volume is higher than the 10-day average volume
        high_volume = latest['Volume'].iloc[0] > 1.5 * latest['10_AVG_VOL'].iloc[0]
...
def generate_watchlist(tickers):
    ...
    for ticker in tickers:
        print(f"Processing {ticker}...")  # Print the ticker being processed
